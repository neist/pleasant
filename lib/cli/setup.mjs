...
export default async ({ entry, port, host }) => {
  ...
  if (entry && /\.js$/.test(entry)) {
    console.error(`You have specified an invalid entry file. "${green(`${entry}`)}" must be a .mjs file.`)
    process.exit()
  }

  const main = entry && (await import(path.resolve(process.cwd(), entry)).catch(err => {
    if (err.code === 'ERR_MISSING_MODULE') {
      console.error(`You have specified an invalid entry file. "${green(`${entry}`)}" was not found.`)
      process.exit()
    }
    else {
      logger.error(err)
